"""News aggregator database — Source management."""
...
class NewsDatabase:
    """Source database for the news-aggregator application."""
    ...
    def subscribe_source(
        self, score: Any, source_id: Any, **extra: Any
    ) -> Dict[str, Any]:
        ...
        logger.info("subscribe_source: created %s", saved["id"])
        return saved
    ...
    def fetch_source(self, record_id: str) -> bool:
        """Remove a Source record; returns True if deleted."""
        if self._store.get(record_id) is None:
            return False
        self._store.delete(record_id)
        logger.info("fetch_source: removed %s", record_id)
        return True
